...
public class MyRenderer implements GLSurfaceView.Renderer {
    ...
    protected Scene _scene;// TODO changed from private to protected
    private GL10 _gl;
    private final TextureManager _textureManager;
    private float _surfaceAspectRatio;
    private final IntBuffer _scratchIntBuffer;
    private final FloatBuffer _scratchFloatBuffer;
    ...
    private final ActivityManager.MemoryInfo _memoryInfo;
    private final float[] proj_matrix;// TODO added
    private MatrixGrabber mg = new MatrixGrabber();// TODO para depurar matrices
    ...
    public void onSurfaceChanged(GL10 gl, int w, int h) {
        ...
        updateViewFrustrum();// TODO puede ser que esto haya que cambiarlo
    }
    ...
    protected void drawSetup() {
        ...
        // gluLookAt modifies the modelview matrix
        GLU.gluLookAt(_gl,
    ...
    public void setProjMatrix(double[] matrix) {
        for (int i = 0; i < 16; i++) {
            Double n = Double.valueOf(matrix[i]);
            proj_matrix[i] = n.floatValue();
        }
    }
    ...
    protected void drawScene() {
        ...
        // TODO added. here works the projection matrix load
        _gl.glMatrixMode(GL10.GL_PROJECTION);
        _gl.glLoadIdentity();
        _gl.glLoadMatrixf(proj_matrix, 0);

        for (int i = 0; i < _scene.children().size(); i++) {
            Object3d o = _scene.children().get(i);
            _gl.glMatrixMode(GL10.GL_MODELVIEW);
//			_gl.glPushMatrix();// save the current matrix and modify a copy of it
//			_gl.glLoadIdentity();
            mg.getCurrentModelView(_gl);
            _gl.glLoadMatrixf(o.modelViewMatrix, 0);
            mg.getCurrentModelView(_gl);
...
//		}
		...
		*/
        ...
        // obtain the modelview matrix from the object and load it
...
//		_gl.glLoadMatrixf(proj_matrix, 0);
    ...
    private void drawAxis() {
        ...
        zAxis = toFloatBuffer(zPoints);
        _gl.glColor4f(0, 1, 0, 0);// green -> x
        _gl.glVertexPointer(3, GL10.GL_FLOAT, 0, xAxis);
        _gl.glDrawArrays(GL10.GL_LINES, 0, 2);

        _gl.glColor4f(1, 0, 0, 0);// red -> y
        _gl.glVertexPointer(3, GL10.GL_FLOAT, 0, yAxis);
        _gl.glDrawArrays(GL10.GL_LINES, 0, 2);

        _gl.glColor4f(0, 0, 1, 0);// blue -> z
        _gl.glVertexPointer(3, GL10.GL_FLOAT, 0, zAxis);
        _gl.glDrawArrays(GL10.GL_LINES, 0, 2);
    }

    private FloatBuffer toFloatBuffer(float[] src) {
        FloatBuffer result;
        ByteBuffer buf = ByteBuffer.allocateDirect(src.length * 4);// 4 bytes per float
