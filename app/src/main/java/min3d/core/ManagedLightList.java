...
public class ManagedLightList {
    ...
    public void reset() {
        ...
        for (int i = 0; i < MyRenderer.NUM_GLLIGHTS; i++) {
            _availGlIndices.add(i);
        }

        _lightToGlIndex = new HashMap<Light, Integer>();

        _glIndexEnabled = new boolean[MyRenderer.NUM_GLLIGHTS];
        _glIndexEnabledDirty = new boolean[MyRenderer.NUM_GLLIGHTS];
        for (int i = 0; i < MyRenderer.NUM_GLLIGHTS; i++) {
    ...
    public boolean add(Light $light) {
        ...
        if (_lights.size() > MyRenderer.NUM_GLLIGHTS)
